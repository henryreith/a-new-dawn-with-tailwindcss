...
class InlineAndMinifyCriticalCssPlugin {
  constructor() {
    ...
    this.liquidOutputPath = path.join(
      __dirname,
      'snippets',
      'critical-css.liquid',
    );
  ...
  apply(compiler) {
    compiler.hooks.afterEmit.tapPromise(
      'InlineAndMinifyCriticalCssPlugin',
      async (compilation, callback) => {
        return new Promise(async (resolve, reject) => {
          // Combine CSS content
          let combinedCss = this.cssFiles
            .map((file) => {
  ...
  return {
    ...
    entry: {
      ...
      'component-free-shipping-bar':
        './src/scss/snippets/free-shipping-bar.scss',
